...
const ImageUploader = () => {
  ...
  const handleFileChange = (event) => {
    const selectedFiles = Array.from(event.target.files).map((file, index) => ({
      ...
      file: file,
      status: 'pendiente', // Estados: pendiente, analizando, listo, error
  ...
  return (
    <div>
      ...
      <div className="space-y-3">
        {files.map(fileData => (
          <FileRow key={fileData.id} fileData={fileData} onDownload={handleDownload} />
        ))}
      </div>
    </div>
  );
  ...
  <div className="bg-gray-700/60 p-3 rounded-lg flex items-center gap-4 transition-all">
    ...
    <div className="action-button w-32 text-right flex-shrink-0">
      {fileData.status === 'listo' && (
        <button onClick={() => onDownload(fileData)} className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold py-2 px-4 rounded-lg transition">
          Descargar
        </button>
      )}
    </div>
